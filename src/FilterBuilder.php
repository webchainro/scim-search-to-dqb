...
class FilterBuilder
{
    ...
    public function __construct(Joiner $joiner)
    {
        $this->joiner = $joiner;
    }

    public function addFiltersOnQueryBuilder(QueryBuilder $queryBuilder, Node $node)
    {
    ...
    {
        $condition = null;
        $nextParameterIndex = count($this->parameters) + 1;
        ...
        switch ($comparisonExpression->operator) {
            case AttributeOperator::EQUAL:
                ...
                $this->parameters[$nextParameterIndex] = $compareValue;
                break;
